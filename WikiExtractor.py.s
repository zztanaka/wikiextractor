...
# zztanaka
# jawikiでは、{{disambig}} だけでなく、{{aimai}} や {{Aimai}} が使われているが、
# 抜けているものもある。その場合でもタイトルに " (曖昧さ回避)" が付加されて
# いるので、 これらを使って曖昧さ回避のページをフィルターする
#filter_disambig_page_pattern = re.compile("{{disambig(uation)?(\|[^}]*)?}}|__DISAMBIG__")
filter_disambig_page_pattern = re.compile("{{[Aa]imai(\|[^}]*)?}}|{{.*曖昧さ回避(\|[^}]*)?}}|{{[Dd]isambig(uation)?(\|[^}]*)?}}|__DISAMBIG__")
filter_disambig_title_pattern = re.compile(" \(曖昧さ回避\)")
...
class Extractor(object):
    ...
    def extract(self, out):
        ...
        # zztanaka
        # self.wiki2text(text) の中のreplaceInternalLinks(text) で
        #   内部リンクを書き換えるが、[[ と ]] の対応が間違っている場合、
        #   その箇所以降の内部リンクが処理されずに残ったままになっている。
        # その間違ってる箇所が headers や lists の中にあったりすると、
        #   compact() の中で、削除されるが、それ以降の部分はそのまま残って
        #   いるので、その部分をここで書き換える。
        text = [self.clean2(replaceInternalLinks(line)) for line in text]
        ...
        for line in text:
            if re.search('。([^」\)\"])', line):
                # 。」や 。) や 。" 以外の場合は、句点の後で改行を挿入
                line = re.sub('。([^」\)\"])', r'。\n\1', line)
                new_text += line.split('\n')
            elif line:  # 空行を削除
                new_text.append(line)
        if len(new_text) > 1:
            for i, line in enumerate(new_text[1:]):
                if line and line[0] == '。':
                    # 行頭に句点がある場合は、前行にアペンド
                    new_text[i] += line[0]
                    # 連続する句点は１つに
                    ...
                    new_text[i+1] = line[1:]
            l = len(new_text)
            for i, line in enumerate(new_text[-2::-1]):
                # (最終-1)行から順に第1行まで
                if line and line[-1] == '、':
                    # 読点で終わる行は、後続する1行をアペンド
                    new_text[l-2-i] += new_text[l-1-i]
                    # 連続する読点は１つに
                    new_text[l-2-i] = re.sub('、+', '、', new_text[l-2-i])
                    new_text[l-1-i] = ''
            new_text = [line for line in new_text if line]  # 空行を削除
        
        text = []
        for line in new_text:
            l = len(line)
            if l > 500:
                continue  # 一行500文字以上のものは除外
            if l == 1:
                continue  # 一行1文字のものは除外
            if line[-1] != '。' and line[-2] != '。':
                continue  # 行末が 。や 。」や 。) や 。" 以外の場合は除外
            if not p_kana_kanji.search(line):
                continue  # 仮名や漢字がないものは除外
            kana_kanji_rate = len(re.findall(p_kana_kanji, line)) / l
            if kana_kanji_rate < 0.2:
                continue  # 仮名や漢字の比率が極めて小さいものは除外
            text.append(line)
        
        # zztanaka
        if len(text) < 2:  # 1行だけの内容しかない場合は除外
            return
        if len(text) > 1000:  # 1,000行を超えるものは、先頭1,000行のみ活用
            text = text[:1000]
    ...
    def transform1(self, text):
        ...
        else:  # options.expand_templates 以外のテンプレートは削除
    ...
    def clean2(self, text):
        text = text.translate(trans_map)  # 制御文字などの置換と削除
        # unicodedata.normalize()は、全角チルダを半角チルダに置換するが、
        # 日本語では全角チルダと半角チルダは別物なので変換したくない。
        # Sentencepieceでもこのように扱っているので、それに合わせる。
        # ジェネレータtildesに、全角と半角チルダの出現順を記憶
        tildes = (c for c in text if c=='～' or c=='~')
        text = unicodedata.normalize('NFKC', text)
        # チルダに関して、unicodedata.normalize()で置換される前の状態に戻す。
        text = ''.join([c if c!='~' else next(tildes) for c in text])

        text = re.sub('[\u2018-\u201B]', '"', text)
        text = re.sub('[\u201C-\u201F]', '"', text)

        text = text.translate(non_bmp_map)  # BMPを超える文字は'〓'に置換
        text = text.translate(ideographic_variation_map)  # 字形選択子を削除
    
        #text = text.translate(non_jis_map)  # 非JISのCJK統合漢字は'鿐'に置換
        #if '鿐' in text:  # 中国語文字列は「〓」に置換
        #    text = re.sub(p_no_jis_str, '〓', text)
        #    text = re.sub(p_multil_con, '〓', text)
        if p_multil.search(text):  # 多言語文字列は「〓」に置換
            text = re.sub(p_multil_str, '〓', text)
            text = re.sub(p_multil_con, '〓', text)
        
        text = re.sub(' +', ' ', text)  # 連続する半角スペースは１つに
        text = text.replace(' 、', '、')  # 句読点の前後の半角スペースを削除
        text = text.replace('、 ', '、')
        text = text.replace(' 。', '。')
        text = text.replace('。 ', '。')
        text = text.replace('、。', '。')  # 句点の前の読点を削除
        text = text.replace(', ,', ',').replace(', .', '.')
        text = re.sub('。+', '。', text)  # 連続する句点は１つに
        text = re.sub('、+', '、', text)  # 連続する読点は１つに
        text = text.replace('、)', ')')  # 括弧内の無用な読点やカンマを削除
        text = text.replace('(、', '(')
        text = text.replace(',)', ')')
        text = text.replace('(,', '(')
        text = text.replace(' (', '(')  # 括弧の前後の半角スペースを削除
        text = text.replace('( ', '(')  # 括弧の前後の半角スペースを削除
        text = text.replace(' )', ')')
        text = text.replace(') ', ')')
        text = text.replace('( )', '')
        ...
        text = text.replace('(〓,)', '')
        text = text.replace('(〓)', '')
        text = text.replace('(〓)', '')  # (〓(〓)) に対処
        text = text.replace('(〓 /)', '')
        text = text.replace('(〓 / )', '')
        text = text.replace('〓 。', '〓。')
        text = text.replace('〓 、', '〓、')
        text = text.replace('()', '')
        text = text.lstrip()  # 行頭のスペースを削除
        text = text.rstrip()  # 行末のスペースを削除
        return text.lower()  # 英文字は小文字化
...
def findBalanced(text, openDelim=['[['], closeDelim=[']]']):
    ...
    """
    # findBalanced(text) で呼ばれた場合、openPat: '\[\['
    openPat = '|'.join([re.escape(x) for x in openDelim])
    # pattern for delimiters expected after each opening delimiter
    afterPat = {o: re.compile(openPat + '|' + c, re.DOTALL) for o, c in zip(openDelim, closeDelim)}  # {'[[': re.compile('\\[\\[|]]', re.DOTALL)}
    stack = []
    start = 0
    cur = 0
    # end = len(text)
    startSet = False  # startポイントがセットされているか
    startPat = re.compile(openPat)  # [[ を見つけるパターン
    nextPat = startPat
    while True:
        ...
        if not startSet:
            start = next.start()
            startSet = True  # startポイントを見つけた
        delim = next.group(0)
        if delim in openDelim:
            stack.append(delim)  # [[ を見つけるごとにスタックに積んでいく
            nextPat = afterPat[delim]  # [[ と ]] の両睨みのパターン
        else:
            opening = stack.pop()
...
# zztanaka
# たとえば、
# {{仮リンク|NISAR (衛星)|en|NISAR (satellite)|label=NISAR}} では、
# args: {'1': 'NISAR (衛星)', '2': 'en', '3': 'NISAR (satellite)', 'label': 'NISAR'}
def ill(args):
    l = args.get('label')
    if l:
        return l
...
# zztanaka
# 「仮リンク」テンプレートの定義を見ると、
# <noinclude>{{Template sandbox notice}}</noinclude><includeonly>{{#invoke:仮リンク|ill}}</includeonly><noinclude>{{Documentation}}</noinclude>
# ここで、
# <noinclude>は、他のページから呼び出されたときに表示しない部分、
# <onlyinclude>は、他のページから呼び出されたときにだけ表示する部分の指定。
# よって、
# options.templates['Template:仮リンク']) -> {{#invoke:仮リンク|ill}}
# となるので、「仮リンク」モジュールの関数illが呼ばれることがわかる。
# そこで、以下で「仮リンク」モジュールと関数illを紐付け、
# 別途、関数illを追加する。
# これをしないと、「仮リンク」テンプレートを展開しようとしても、空文字列に
# 置き換わるだけとなる。
modules = {
    'convert': {
...
def replaceInternalLinks(text):
    ...
    for s, e in findBalanced(text):
        ...
        if pipe < 0:
            ...
            # zztanaka
            # label が空文字列の場合は、titleを当てる
            # たとえば、元々の [[有声両唇軟口蓋接近音|{{IPA|j}}]] が
            # self.transform(text) の中の transform1() で {{IPA|j}} が
            # 削除されて、[[有声両唇軟口蓋接近音|]] となっていまい、
            # このままだと、空の label が優先されて、この箇所が空文字列に
            # 置換されてしまう。これを防ぐために、label = title にする。
            if label == '':
                label = title
...
def makeInternalLink(title, label):
    ...
    if colon > 0 and title[:colon] not in options.acceptedNamespaces:
        return ''  # Image: や File: や Category: などのリンクは削除
    if colon == 0:
        # drop also :File:
...
class NextFile(object):
    ...
    def __next__(self):
        ...
        if self.file_index == 0:  # filesPerDirごとにディレクトリをインクリメント
            self.dir_index += 1
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    for line in input:
        ...
        if not m:
            continue
        tag = m.group(2)  # タグ名を抽出
        if tag == 'base':
            # discover urlbase from the xml dump file
            # /mediawiki/siteinfo/base
            base = m.group(3)  # https://ja.wikipedia.org/wiki/メインページ
            options.urlbase = base[:base.rfind("/")]  # https://ja.wikipedia.org/wiki
        elif tag == 'namespace':
            # https://ja.wikipedia.org/wiki/Help:名前空間 を参照
            mk = keyRE.search(line)
            if mk:
                nsid = ''.join(mk.groups())  # 名前空間のID番号
            else:
                nsid = ''  # key="(\d*)"にマッチしない、-1 と -2 はこっち
            options.knownNamespaces[m.group(3)] = nsid  # m.group(3)は名前空間名
            if re.search('key="10"', line):
                options.templateNamespace = m.group(3)  # Template
                options.templatePrefix = options.templateNamespace + ':'
            elif re.search('key="828"', line):
                options.moduleNamespace = m.group(3)  # モジュール
                options.modulePrefix = options.moduleNamespace + ':'
        elif tag == '/siteinfo':  # </siteinfo> まででfor文終了
            break

    if options.expand_templates:  # ここは False
        # preprocess
    ...
    # zztanaka
    # 本来の2パスのtemplate展開の処理を無視して、強制的に以下のテンプレート
    # だけを展開させることにする。
    #for key in options.templates.keys():
    #    if re.match('Template:Lang', key):
    #        print(key)
    #        print(options.templates[key])
    options.templates = {
        'Template:仮リンク': '{{#invoke:仮リンク|ill}}',
        'Template:要出典': '{{{1}}}',
        'Template:要出典範囲': '{{{1}}}',
        'Template:Lang': '<span lang="{{{1}}}" xml:lang="{{{1}}}">{{{2}}}</span>{{#if:{{{3|}}}|&#32;<small>{{{3}}}</small>}}',
        'Template:LangWithName': '{{#ifeq:{{{links|}}}|no |{{{2}}} |[[{{{2}}}]]}}&#58; {{Lang|{{{1}}}|{{{3}}}}}',
        'Template:Lang-en-short':
        '{{langWithName|en|{{#ifeq:{{{links|}}}|no|英|英語{{!}}英}}|{{{1|}}}|links={{{links|yes}}}}}',
        'Template:Lang-us-short':
        '{{langWithName|en-us|アメリカ英語{{!}}米|{{{1|}}}|links={{{links|{{{link|yes}}}}}}}}'
    }
    ...
    # zztanaka added
    if (out_file):
        new_page_articl_used_total = 0
        for dir in glob.glob(os.path.join(out_file, '[A-Z][A-Z]')):
            files = sorted(glob.glob(os.path.join(dir, 'wiki_*')))
            all_file = os.path.join(dir, 'all.txt')
            ...
            with open(all_file, 'w') as outfile:
                for file in files:
                    with open(file) as infile:
                        for line in infile:
                            if re.match('$', line):
                                new_page_articl_used_total += 1
                            outfile.write(line)
        logging.info(f'real total of used articl page: {new_page_articl_used_total:d}')
...
def main():
    ...
    try:
        ...
        file_size = int(args.bytes[:-1]) * 1024 ** power  # 500M -> 524288000
...
if __name__ == '__main__':
    # zztanaka
    remove_tags = (r'<ruby[^>]*>', r'</ruby>',
                   r'<rb>', r'</rb>', r'<rp>', r'</rp>', r'<rt[^>]*>', r'</rt>',
                   r'<nowiki>', '</nowiki>', r'<br>', r'</br ?>',
                   r'<del>', r'</del>', r'<ins>', r'</ins>', r'<ce>', r'</ce>',
                   r'<chem>', r'</chem>', r'<math>', r'</math>',
                   r'<small>', r'</small>', r'<onlyinclude>', r'</onlyinclude>',
                   r'<includeonly>', r'</includeonly>',
                   r'<sup>', r'</sup>', r'<sub>', r'</sub>',
                   r'<quote>', r'</quote>', r'<samp>', r'</samp>',
                   r'<q>', r'</q>', r'<q [^>]+>',
                   r'<ref[^>]*>', r'</ref[^>]*>',
                   r'<REF[^>]*>', r'</REF[^>]*>',
                   r'^.*<html.*', r'^.*</html>.*',
                   r'<section[^>]*>', r'</table>',
                   r'<div[^>]*>', r'</div[^>]*>',
                   r'</graph>', r'^<mapframe.*$', r'</mapframe>', 
                   r'<score[^>]*>', r'</score>', r'<poem[^>]*>', r'</poem>',
                   r'<ref [^/]+/', r'</u >',
                   r'<span>', r'</span[^>]*>',
                   r'<dfn>', r'</dfn>', r'<rf[^>]+>', r'</rf>')
    # url
    p_url = re.compile(r'https?://[\w/:%#\$&\?\(\)~\.=\+\-]+', re.IGNORECASE)
    # 仮名やCJK統合漢字
    p_kana_kanji = re.compile(r'[ぁ-んァ-ヶ\u4E00-\u9FD0]')
    # ハングル文字を含む多言語文字など（注: 全角文字や半角カタカナの領域も
    # 入っているが、'～'(U+FF5E)以外はNFKC標準化後に用いるので問題ない）
    p_multil = re.compile(r'[\u0400-\u1FFF\u2C00-\u2DDF\u3100-\u31FF\uA000-\uF8FF\uFB00-\uFF5D\uFF5F-\uFFDF]')
    p_multil_str = re.compile(r'[a-zA-Z\u0100-\u1FFF\u2C00-\u2DDF\u3100-\u31FF\uA000-\uF8FF\uFB00-\uFF5D\uFF5F-\uFFDF]*[\u0100-\u1FFF\u2C00-\u2DDF\u3100-\u31FF\uA000-\uF8FF\uFB00-\uFF5D\uFF5F-\uFFDF]+[.a-zA-Z\u0100-\u1FFF\u2C00-\u2DDF\u3100-\u31FF\uA000-\uF8FF\uFB00-\uFF5D\uFF5F-\uFFDF]*')
    p_multil_con = re.compile(r'(〓[ -])+〓')

    # BMP (Unicodeの基本多言語面[U+0000 - U+FFFF]) を超える文字は'〓'に置換
    #non_bmp_map = dict.fromkeys(range(0x10000, sys.maxunicode + 1), '〓')
    non_bmp_map = dict.fromkeys(range(0x10000, 0x2ffff + 1), '〓')
    # 字形選択子を削除
    ideographic_variation_map = dict.fromkeys(range(0xe0100, 0xe01ef + 1), '')

    ## (CJK統合漢字でcp932で変換できない) JIS第一第二水準(+MS拡張)漢字以外は
    ## そのような文字を代表させて'鿐'(U+9FD0)に置換
    #non_jis_cjk_unified = [u for u in range(0x4e00, 0x9fd0 + 1) if chr(u).encode('cp932','ignore').decode('cp932')=='']
    #non_jis_map = dict.fromkeys(non_jis_cjk_unified, '鿐')
    ## 非JIS漢字とJIS漢字からなる文字列を抽出する
    #p_no_jis_str = re.compile(r'[\u4E00-\u9FCF]*\u9FD0+[\u4E00-\u9FCF]*')

    trans_map = str.maketrans(
        ''.join(['\u1160',  # ハングル字母のスペースの置換
                 '\u3164',  # ハングル互換字母のスペースの置換
                 '\u2800',  # 点字のスペースの置換
                 '\u1680',  # OGHAM SPACE MARK
                 '\u200B',  # ZERO WIDTH SPACE
                 ...
                 '\u2028',  # LINE SEPARATOR
                 '\u2029',  # PARAGRAPH SEPARATOR
                 '\u2581',  # LOWER ONE EIGHT BLOCK（Sentencepieceで半角スペース置換に使用）
                ...
                ['\u2010',  # HYPHEN
                 ...
                 '\u2212'] +  # MINUS SIGN
                ['\u318D']),  # ハングル互換字母の中点の置換
        ' '*13 + '-'*7 + '・',  # 置換先は、SPACEとHYPHEN-MINUSと中点
        ''.join([chr(i) for i in range(0, 0x08+1)] +  # HT以外の制御文字を削除
                [chr(i) for i in range(0x0a, 0x1F+1)] +  # 制御文字を削除
                [chr(i) for i in range(0x7f, 0xa1+1)] +  # 制御文字を削除
                ['\u00AD',  # SOFT HYPHONを削除
                 '\uFE0E',  # 絵文字の異体字選択符号を削除
                 '\uFE0F',  # 絵文字の異体字選択符号を削除
                 '\uFEFF',  # BOMを削除
